// Unit tests for processCallPost controller
// Why: validate input handling, schema/retry logic, env behavior, and error paths without calling real LLM
...
// Helper to build an isolated app mounting only /process-call and standard middlewares
function buildApp() {
  const app = express();
...
// A valid AI output sample matching callExtractionSchema
...
describe('processCallPost controller', () => {
  ...
  beforeEach(() => {
    ...
    process.env.MAX_TRANSCRIPT_LEN = '1000'; // default for most tests
  });
  ...
  it('retry once: first invalid schema then valid -> 200', async () => {
    // First response invalid (missing required properties), second valid
    mockedProcess
    ...
    // Verify repair prompt shape contains original transcript and hint
  ...
  it('env handling: missing MAX_TRANSCRIPT_LEN -> no length cap applied (documents current behavior)', async () => {
    // Note: Number(undefined) -> NaN, making (length > NaN) false, so no cap applied
